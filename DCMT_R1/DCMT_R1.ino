...
const float VREF_V = 5.0;        // ADC reference voltage
const float CLEAR_RAW = 250.0;   // clear/tap baseline
const float CLOUDY_RAW = 180.0;  // baking soda reference
const float TURB_ALPHA = 0.15f;  // smoothing factor
float turb_ema1 = NAN;
float turb_ema2 = NAN;
const float ALPHA_SLOW  = 0.12f;  // smoothing for small noise (0.05–0.2 typical)
const float STEP_THRESH = 80.0f; 
static const float CAL_CLEAR_V   = 3.70f;   // tap (clear) volts
static const float CAL_CLOUDY_V  = 3.40f;   // baking-soda volts
static const float CAL_CLOUDY_NTU= 100.0f;  // assign e.g. 100 NTU for your mix


static inline float voltsToNTU_linear(float v) {
  const float m = -CAL_CLOUDY_NTU / (CAL_CLEAR_V - CAL_CLOUDY_V);  // negative slope
  float ntu = m * (v - CAL_CLEAR_V);
  if (ntu < 0) ntu = 0;         // keep non-negative
  return ntu;
}
...
unsigned long lastPassiveTurbSample = 0;
const unsigned long PASSIVE_SAMPLE_MS = 200; // ~5 Hz
...
// --- Oversample the ADC quickly and average (fast + low-noise, no lag) ---
#ifndef BURST_SAMPLES
#define BURST_SAMPLES 16   // try 8/16/32 depending on noise vs CPU time
#endif

static inline float burstAverageVolts(uint8_t analogPin, uint8_t samples = BURST_SAMPLES) {
  // One throwaway read to let the mux/S&H settle after channel changes
  ...
  for (uint8_t i = 0; i < samples; i++) {
    ...
    delayMicroseconds(120); // tiny pause helps the S/H cap recharge
  }
  float raw = acc / (float)samples;        // 0..1023
  return (raw / 1023.0f) * VREF_V;         // volts
}
...
void loop() {
  ...
  // Passive sampling when turbidity pumps are OFF, so the website always updates
if (!DCMT.turbPump1 && !DCMT.turbPump2) {
  if (millis() - lastPassiveTurbSample >= PASSIVE_SAMPLE_MS) {
    lastPassiveTurbSample = millis();

    DCMT.turbVoltage[0] = burstAverageVolts(ENC_B1, BURST_SAMPLES);
    DCMT.turbVoltage[1] = burstAverageVolts(ENC_B2, BURST_SAMPLES);

     DCMT.currentTurbidity[0] = voltsToNTU_linear(DCMT.turbVoltage[0]);  // NTU
    DCMT.currentTurbidity[1] = voltsToNTU_linear(DCMT.turbVoltage[1]);  // NTU
  }
}
  printOutput();
}
...
void measureTurbidity() {
  if (DCMT.turbPump1) {
    ...
    // ---- Pump 1 measure block (state == 2) ----
if (turbidityCycle1 == 2 && (millis() - lastTurbiditySequence1 >= TURBIDITY_SAMPLE_WAIT_MS)) {
  lastTurbiditySequence1 = millis();
  turbidityCycle1++;

  // --- turbidity measurement for pump 1 ---
float volts1 = burstAverageVolts(ENC_B1, BURST_SAMPLES);
DCMT.turbVoltage[0]      = volts1;
DCMT.currentTurbidity[0] = voltsToNTU_linear(volts1);  // NTU for site

Serial.print("Sample 1 measured! V:");
Serial.print(volts1, 3);
Serial.print(" NTU:");
Serial.println(DCMT.currentTurbidity[0], 1);


  DCMT.m1Dir = 0;
  DCMT.m1Speed = TURBIDITY_PUMP_SPEED * 255.0 / 100.0;
  actuateMotors();
...
if (turbidityCycle1 == 3 && (millis() - lastTurbiditySequence1 >= TURBIDITY_PUMP_TIME_R_MS))  // reverse sample for some time
  ...
  if (DCMT.turbPump2) {
    ...
    // ---- Pump 2 measure block (state == 2) ----
if (turbidityCycle2 == 2 && (millis() - lastTurbiditySequence2 >= TURBIDITY_SAMPLE_WAIT_MS)) {
  lastTurbiditySequence2 = millis();
  turbidityCycle2++;

  // --- turbidity measurement for pump 2 ---
float volts2 = burstAverageVolts(ENC_B2, BURST_SAMPLES);
DCMT.turbVoltage[1]      = volts2;
DCMT.currentTurbidity[1] = voltsToNTU_linear(volts2);  // NTU for site

Serial.print("Sample 2 measured! V:");
Serial.print(volts2, 3);
Serial.print(" NTU:");
Serial.println(DCMT.currentTurbidity[1], 1);


  DCMT.m2Dir = 0;
  DCMT.m2Speed = TURBIDITY_PUMP_SPEED * 255.0 / 100.0;
  actuateMotors();
...
void actuatePumps() {
  ...
  // Motor 1 (Acid Pump)
  if (millis() - m1StartTime > PUMP_PULSE_PERIOD_MS) {  //time to shift the Relay Window
  ...
  // Motor 2 (Base Pump)
  if (millis() - m2StartTime > PUMP_PULSE_PERIOD_MS) {  //time to shift the Relay Window
...
void printOutput() {
  if (millis() - lastSerialPrint >= SERIAL_UPDATE_TIME_MS) {
    Serial.print("V1:");  Serial.print(DCMT.turbVoltage[0], 3);
    Serial.print(" NTU1:"); Serial.print(DCMT.currentTurbidity[0], 1);
    Serial.print("  V2:");  Serial.print(DCMT.turbVoltage[1], 3);
    Serial.print(" NTU2:"); Serial.print(DCMT.currentTurbidity[1], 1);
    Serial.println();
...
void requestEvent() {
  FLOATUNION_t n1, n2;
  n1.number = isnan(DCMT.currentTurbidity[0]) ? 0.0f : DCMT.currentTurbidity[0];
  n2.number = isnan(DCMT.currentTurbidity[1]) ? 0.0f : DCMT.currentTurbidity[1];
  for (int i = 0; i < 4; i++) Wire.write(n1.bytes[i]);
  for (int i = 0; i < 4; i++) Wire.write(n2.bytes[i]);
}
